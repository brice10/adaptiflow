 ...
 */
public class IncreaseEventTest {
    private TestMetricsCollector collector;
    private IncreaseEvent<Double> increaseEvent;
    ...
    public void setUp() {
        ...
        increaseEvent = new IncreaseEvent<>(collector, thresholdProvider);
        observer = new TestObserver<>();
        increaseEvent.subscribe(observer);
    }

    @Test
    public void testNotifyObserverOnIncreaseEvent() {
        collector.setValue(70.0); // Value exceeds threshold
        increaseEvent.observe();

        Assert.assertEquals(1, observer.getNotifiedValues().size());
        Assert.assertEquals(70.0, observer.getNotifiedValues().get(0), 0.01);
    }

    @Test
    public void testNoNotificationForDecrease() {
        collector.setValue(40.0); // Below the threshold
        increaseEvent.observe();
